...
class KritaStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the krita file path if one
    does not exist.
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "Krita '%s' plugin rejected the current session..." % (self.name,)
                )
                self.logger.info("  There is already a version number in the file...")
                self.logger.info("  Krita file path: %s" % (path,))
            ...
            self.logger.warn(
                "The Krita session has not been saved.", extra=_get_save_as_action()
            )

        self.logger.info(
            "Krita '%s' plugin accepted the current session." % (self.name,),
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            # validation fails
            error_msg = "The Krita session has not been saved."
    ...
    def publish(self, settings, item):
        ...
        self.logger.info("A version number has been added to the Krita file...")
        self.logger.info("  Krita file path: %s" % (version_path,))
